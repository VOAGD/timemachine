...
def gradient_200(weights, dev):
    ...
    def PSR_gradients(gradient):
            psr_g_plus = circuit(theta + np.pi / 2)
            psr_g_minus = circuit(theta - np.pi / 2)
            
            return ((psr_g_plus - psr_g_minus)/ 2)
        
    def PSR_hessian(hessian):
            psr_h_plus = circuit(theta + np.pi / 2)
            psr_h_minus = circuit(theta - np.pi / 2)
            
            return ((psr_h_plus - psr_h_minus)/ 2)
